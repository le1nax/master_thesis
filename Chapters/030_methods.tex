...
In this chapter, we introduce our approaches to tackle robust cell segmentation in 3D. First, we present all datasets used for pretraining, finetuning and evaluation of our resulting models, as the inductive bias incroperated from training has been shown to be as important as architectual design choices, to achieve generalizable models \cite{Pachitariu.2025}. For network design, we first sketch the outline of state-of-the-art networks on a high level, to present our architectual design choices made more intuitive. After inctroducing our proposed network designs, we dive into the training pipeline that we integrate for model pretraining and finetuning. Apart from the default pipeline, we will also propose a way to handle partially annotated training data. Finally, we will also discuss methods for true 3D training. 
\section{Datasets}
In this section, we introduce some important datasets used in this work for model training and performance evaluation. First, we present every datasets involved in pretraining of our models. After that, we introduce datasets for finetuning and performance evaluation.
...
\begin{figure}[!ht]
    ...
    \caption{Encoder embeddings of the train set clustered with k-mean and intuition of amplified sampling \cite{LeCun.1989}.}
    \label{fig:slicing}
\end{figure}
