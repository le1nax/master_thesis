...
The expansive path, on the other hand, restores the spatial resolution through a sequence of upsampling operations (e.g., transposed convolutions) and concatenations with corresponding feature maps from the contracting path via \textit{skip connections}. These skip connections provide fine-grained spatial details from earlier layers, improving the precision of the segmentation boundaries.
...
The IoU score, also known as the Jaccard index, measures the overlap between the predicted mask $P$ and the ground truth mask $G$ relative to their union:
\[
...
TODO!!!!! For a $\mathrm{IoU}$ threshold $\tau$, the $\mathrm{AP}_{\tau}$ is defined as:
%
\begin{equation}
\label{eq:ap_tau}
\mathrm{AP}_{\tau} = \frac{\mathrm{TP}_{\tau}}{\mathrm{TP}_{\tau} + \mathrm{FN}_{\tau} + \mathrm{FP}_{\tau}},
\end{equation}
%
given the true positive $(\mathrm{TP}_{\tau})$, false positive $(\mathrm{FN}_{\tau})$ and false negative $(\mathrm{FP}_{\tau})$ rates at the corresponding $\mathrm{IoU}$ threshold $\tau$ are given.

This is particularly useful to compare the segmentation performance in environments where annotations can not be manually be created with certainty. Therefore it is useful to compare the AP at a threshold of 0.5 for example, if contours are really noisy. Also, this measure gives simple feedback in terms of high accuracy performance. For example, it has been shown, that Stardist has an overall higher accuracy than the U-Net, but when it comes to high accuracy predictions at an AP threshold of 0.9, Stardist's perfromance actually stagnates \cite{Schmidt_2018}.
...
\begin{figure}[!ht]
    ...
    \caption{Encoder self-attention for a set of reference points. The encoder is able to separate individual instances (A-D), adapted from \cite{Carion.2020}. }
    \label{fig:attention}
\end{figure}

\subsection{Transformer}

Building on the attention mechanism, the transformer architecture was originally introduced for sequence modeling tasks in natural language processing, but has since proven highly versatile across domains \cite{Carion.2020} \cite{dosovitskiy2021imageworth16x16words}. Its core innovation lies in replacing recurrent and convolutional operations with a stack of self-attention and feedforward layers, enabling efficient modeling of long-range dependencies.
...
Considering they basically reimplemented the concepts of Sequeeze and Exitement (SE) \cite{hu2019squeezeandexcitationnetworks} layers along with some concatinations and additions from two resolutions, the term attention may be a little misleading here. Nevertheless this way of scaling feature maps with fused SE results has been proven to perform really well. Also, a very similar approach was implemented in the original Cellpose paper, where they took the gloabl average pools from the bottleneck feature map and propergated this vector (refered to as \textit{styles}) along shallower layers of the decoder \cite{Stringer.2020}. 
